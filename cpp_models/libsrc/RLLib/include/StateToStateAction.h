  ...
  class StateActionTilings: public StateToStateAction<T>
  {
    ...
    public:
      StateActionTilings(Projector<T>* projector, Actions<T>* actions) :
          projector(projector),
          actions(actions),
          phis(new Representations<T>(projector->dimension(), actions))
      {
      }
      ...
      /**
       * Returns the projected something?
       */
      const Vector<T>* stateAction(const Vector<T>* x, const Action<T>* a)
      {
        if (this->actions->dimension() == 1)  // why would there be only one action?
          return this->projector->project(x);  // projection from whole space
        else
          return this->projector->project(x, a->id());
      }

      const Representations<T>* stateActions(const Vector<T>* x)
      {
        ASSERT(this->actions->dimension() == this->phis->dimension());
        if (x->empty())
        {
          this->phis->clear();
          return this->phis;
        }
        for (auto a = this->actions->begin(); a != this->actions->end(); ++a)
          this->phis->set(stateAction(x, *a), *a);
        return this->phis;
      }
