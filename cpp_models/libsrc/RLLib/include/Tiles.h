...
namespace RLLib
{
  ...
  class Tiles
  {
    protected:
      ...
      int coordinates[Hashing<T>::MAX_NUM_VARS * 2 + 1];

      Hashing<T>* hashing; /* The hash function */

      Vector<int>* i_tmp_arr;
      Vector<T>* f_tmp_arr;

      /**
       * Called at the beginning of tiling process.
       * Initializes this->qstate and this->base.
       */
      void initialize_float_quantization(const int& num_tilings,
                                         const Vector<T>* floats,
                                         const int& num_floats)
      {
        for (int i = 0; i < num_floats; i++)
        {
          this->qstate[i] = (int) floor(floats->getEntry(i) * num_tilings);
          this->base[i] = 0;
        }
      }

      /**
       * Called at each step of tiling process.
       * Recomputes new quantized values for float coordinates.
       * Overwrites those values to this->coordinates.
       */
      void quantize_floats(const int& num_tilings, const int& num_floats)
      {
        for (int i = 0; i < num_floats; i++)
        {
          // Find coordinates of activated tile in tiling space
          if (this->qstate[i] >= this->base[i])
          {
            int diff = this->qstate[i] - this->base[i];
            this->coordinates[i] = this->qstate[i] - (diff % num_tilings);
          }
          else
          {
            int diff = this->base[i] - this->qstate[i];
            this->coordinates[i] = this->qstate[i] + 1
                                   + ((diff - 1) % num_tilings)
                                   - num_tilings;
          }
    ...
    public:
      Tiles(Hashing<T>* hashing) :
          hashing(hashing),
          i_tmp_arr(new PVector<int>(Hashing<T>::MAX_NUM_VARS)),
      ...
      ~Tiles()
      {
        delete this->i_tmp_arr;
        delete this->f_tmp_arr;
      }

      /**
       * Computes 'num_tilings' integer coordinates based on 'floats' and 'ints'.
       * Computes hash values for those coordinates.
       * Sets elements of 'the_tiles' corresponding to hash values to 1.0f.
       */
          ...
          int num_ints)  // number of integer variables
      {
        ...
        for (int i = 0; i < num_ints; i++)
          this->coordinates[num_floats + 1 + i] = ints->getEntry(i);

        this->initialize_float_quantization(num_tilings, floats, num_floats);

        // Compute tile hash values
        for (int j = 0; j < num_tilings; j++)
        {
          this->quantize_floats(num_tilings, num_floats);
          // Add additional indices for tiling and hashing_set so they hash differently
          coordinates[num_floats] = j;
          int hash = this->hashing->hash(this->coordinates, num_coordinates);
          the_tiles->setEntry(hash, 1.0f);
        }
      }
